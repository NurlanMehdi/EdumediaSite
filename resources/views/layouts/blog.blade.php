        ...
        <div class="filter_header">
            <ul class="filter_header_nav">
                <li class="filter_header_nav_item">
                    <a class="filter_header_nav_item_link blog-name all-blogs" id="0">All</a>
                </li>
                @foreach($blogItems as $item)
                    <li class="filter_header_nav_item">
                        <a class="filter_header_nav_item_link blog-name" id="{{$item->item_id ?? ''}}">{{$item->blog_name ?? ''}}</a>
                    </li>
                @endforeach
            </ul>
        </div>
    ...
    <script>
        $(document).ready(function (){
            ...
            $('.blog-name').on('click',function (){
                ...
                var id = $(this).attr('id');
                let url = "{{route('post.selected.from.blog',':id')}}";
